...
impl MusicFile {
  fn new(dir_path: String) -> MusicFile {
    MusicFile { dir_path , music_files: Vec::new()}
  }
  ...
  fn filter_music_files(&self ,file : ReadDir) -> Vec<String> {
    ...
    for file in file {
      ...
      if path.extension().unwrap() == "wav" || path.extension().unwrap() == "mp3" || path.extension().unwrap() == "ogg" || path.extension().unwrap() == "flac" || path.extension().unwrap() == "aac" || path.extension().unwrap() == "m4a" || path.extension().unwrap() == "wma" || path.extension().unwrap() == "aiff" || path.extension().unwrap() == "alac" || path.extension().unwrap() == "dsd" || path.extension().unwrap() == "egg" {
...
pub fn get_bs_music_files() -> Vec<String> {
  ...
  file_list.get_music_dirs_files();
